...
app.post("/api/teams", async (req, res) => {
  try {
    console.log("Request Body:", req.body);
    const { user_id, name, pokemon1, pokemon2, pokemon3, pokemon4, pokemon5, pokemon6 } = req.body;
    // Validate required fields
    if (!user_id || !name || !pokemon1 || !pokemon2 || !pokemon3 || !pokemon4 || !pokemon5 || !pokemon6) {
      return res.status(400).json({
        error:
    ...
    const { data: team, error } = await supabase
      .from("teams")
      .insert([{ user_id, name, pokemon1, pokemon2, pokemon3, pokemon4, pokemon5, pokemon6 }]);
...
app.post("/api/sign-in", async(req, res) => {
  try {
    ...
    // Compare passwords
    const isPasswordMatch = await bcrypt.compareSync(password, user.password_hash);

    isPasswordMatch ? res.status(200).json({
...
app.post("/api/sign-up", async (req, res) => {
  try {
    ...
    if (password !== passwordConfirmation) {
      return res.status(400).json({
        error: "Bad Request: Please make sure that password confirmation and password match",
      });
    }
